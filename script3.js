...
function createLights() {
  ...
  directionallight = new THREE.DirectionalLight(0xfffff, 0.0, 01);
...
EnnemiesHolder.prototype.spawnEnnemies = function () {
  ...
  for (var i = 0; i < nEnnemies; i++) {
    ...
    // this.EnnemiesInUse.push(ennemy);,было так
    this.ennemiesInUse.push(ennemy); //стало так
  }
};
...
Particle = function () {
  var geom = new THREE.TetrahedronGeometry(3, 1);
  var mat = new THREE.MeshPhongMaterial({
    color: Colors.red,
    specular: 0xffffff,
    skinning: 0,
...
Particle.prototype.explode = function (poz, color, scale) {
  var _this = this;
  var _p = this.mesh.parent;
  this.mesh.material.color = new THREE.Color(color);
  this.mesh.material.needsUpdate = true;
  this.mesh.scale.set(scale, scale, scale);
  var targetX = poz.x + (-1 + Math.random() * 2) * 50;
  var targetY = poz.y + (-1 + Math.random() * 2) * 50;
...
ParticlesHolder.prototype.spawnParticles = function (
  ...
  for (var i = 0; i < nParticles; i++) {
    ...
    var _this = this;
    particle.mesh.position.y = poz.y;
    particle.mesh.position.x = poz.x;
    particle.explode(poz, color, scale);
  }
};
...
function loop() {
  ...
  ambientLight.intensity += (0.5 - ambientLight.intensity) * deltaTime * 0.005; // Добавил эту строку
...
function updatePlane() {
  ...
  airplane.mesh.rotation.x = (airplane.mesh.position.y - targetY) * 0.01; //не было этой строки
...
function handleMouseUp(event) {
  if (game.status == "waitingReplay") {
    ...
    for (var i=0; i<coinsHolder.coinsInUse.length; i++){
      const coins = coinsHolder.coinsInUse[i];
      coinsHolder.mesh.remove(coins.mesh)
    }
...
window.addEventListener("load", function(){
  this.window.document.querySelector("button.start-game").addEventListener("click", function(){
    init();
